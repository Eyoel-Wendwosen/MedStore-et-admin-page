...
class Message extends React.Component {
	...
	async componentDidMount() {
		let messages = await axios.get(`${SERVER_URL}${API_URL}/message?processed=0`)
	...
	handelDelete(e) {
		e.preventDefault()
		axios.delete(`${SERVER_URL}${API_URL}/message/${this.state.selectedMessage.id}`).then(() => {
	...
	async handleProcess(e) {
		...
		let res = await axios.patch(
			`${SERVER_URL}${API_URL}/message/${this.state.selectedMessage.id}`,
			{
				processed: true,
			}
		)
		this.setState({
			messages: this.state.messages.filter(message => message.id != res.data.data._id),
		})
	}
